...
void APlayerCharacter::BeginPlay()
{
	...
	AirCurrentMontage = AirComboMontageArr[0];	//初始化空中连击Montage
...
void APlayerCharacter::PlayerCombo()
{
	...
	//上挑攻击
	else if (PlayerLockMode == EPlayerStateMode::ELOCK && bLShiftIsPressed && InputY == -1.f && !PlayerIsInAir())//判断上挑攻击释放条件
	{
	...
	//下落攻击
	else if (bLShiftIsPressed && InputY == -1.f && PlayerIsInAir())
	{
		...
		if (dis >= 50.f)
		{
			...
			if (PlayerCurrentMode == EPlayerStateMode::EACK)
			{
				...
				else if(AnimInstance->Montage_IsPlaying(AirCurrentMontage))
					AnimInstance->Montage_Stop(0.f, AirCurrentMontage);
				else
					return;
			}
	...
	else if (PlayerIsInAir())
	{
		...
		if ((GetDisWithVector(FVector(GetActorLocation().X, GetActorLocation().Y, 0.f)) - 90.f) >= 50.f)
		{
			if (PlayerCurrentMode == EPlayerStateMode::EACK && AnimInstance->Montage_IsPlaying(UpperACKMontage))
				AnimInstance->Montage_Stop(0.f, UpperACKMontage);

			if (AnimInstance && AirCurrentMontage)
			{
				PlayerStateTemp->Set_ACKMode(PlayerCurrentMode);
				CurrPlayerACKMode = EPlayerACKMode::EAirACKMode;
				if (AnimInstance->Montage_IsPlaying(AirCurrentMontage))
					PlayNextMontage(AirComboMontageArr, AirCurrentMontage);
				MontageEndedDelagate.BindUObject(this, &APlayerCharacter::AttackEnded);
				AnimInstance->Montage_Play(AirCurrentMontage);
				HitAIType = EHitAIACKType::ComboHit;
				AnimInstance->Montage_SetEndDelegate(MontageEndedDelagate);
				bIsAttacking = true;
				TakeEnergy(15.f);
		...
		{
			if (AnimInstance && AnimInstance->Montage_IsPlaying(AirCurrentMontage))
			{
				AnimInstance->Montage_Stop(0.1f, AirCurrentMontage);
	...
	else if(!PlayerIsInAir())
	{
		TargetEnemy.Empty();
		//上段攻击
		if (AnimInstance && !AnimInstance->Montage_IsPlaying(PlayerComboACK) && CurrPlayerACKMode == EPlayerACKMode::EHighPoseMode)
		{
		...
		//中段攻击
		else if (AnimInstance && !AnimInstance->Montage_IsPlaying(MidAckMontage) && CurrPlayerACKMode == EPlayerACKMode::EMidPoseMode)
		{
			//如果此时玩家处于完美闪避状态那么此时普通攻击产生吸附
			if (bIsPerfectSliding)
			{
		...
		else if (AnimInstance && bIsAttacking && (CurrPlayerACKMode == EPlayerACKMode::EHighPoseMode || CurrPlayerACKMode == EPlayerACKMode::EMidPoseMode))
		{
			PlayerStateTemp->Set_ACKMode(PlayerCurrentMode);
			ChangeCombatSection();
		}
...
{
	if (AnimInstance->Montage_IsPlaying(PlayerComboACK) && CurrPlayerACKMode == EPlayerACKMode::EHighPoseMode && bCanCombo)
	{
	...
	else if (AnimInstance->Montage_IsPlaying(MidAckMontage) && CurrPlayerACKMode == EPlayerACKMode::EMidPoseMode && bCanCombo)
	{
		FName CurrentCombatSection = AnimInstance->Montage_GetCurrentSection();
	...
	if (OutHits.Num() != 0)
	{
		for (auto& result : OutHits)
		{
			...
			if (EnemyTemp)
			{
				...
				//if (CurrPlayerACKMode == EPlayerACKMode::EHighPoseMode && HighCurrMotage)
				//	CombatStrikeSet(HighCurrMotage);
				//else if(CurrPlayerACKMode == EPlayerACKMode::EMidPoseMode && MidCurrMontage)
				//	CombatStrikeSet(MidCurrMontage);
				///**/
...
{
	if (AnimInstance->Montage_IsPlaying(AirDownACK_Montage) || AnimInstance->Montage_IsPlaying(AirCurrentMontage) || 
...
void APlayerCharacter::PlayNextMontage(const TArray<UAnimMontage*>& MontageArray, UAnimMontage* CurrentAnimMontage)
{
	...
	{
		if (MontageArray[i] == CurrentAnimMontage && i != MontageArray.Num())
		{
			//DebugNS::DurationDebug(3.f, FString("CheckOut2Montage!"));
			CurrentAnimMontage = MontageArray[i + 1];
			AnimInstance->Montage_Play(CurrentAnimMontage);
			bIsAttacking = true;
			return;
		}
		else if (MontageArray[i] == CurrentAnimMontage && i == MontageArray.Num())
		{
			//DebugNS::DurationDebug(3.f, FString("CheckOut1Montage!"));
...
void APlayerCharacter::AirFlashSapwnShadow()
{
	...
	else if (PlayerIsInAir() && AnimInstance->Montage_IsPlaying(AirCurrentMontage))
	{
		GetWorldTimerManager().ClearTimer(FlashShadowHandler);
		return;
	}
...
{
	if (CurrPlayerACKMode != EPlayerACKMode::EUpperACKMode && CurrPlayerACKMode != EPlayerACKMode::EDownACKMode && 
		CurrPlayerACKMode != EPlayerACKMode::EAirACKMode && bIsAttacking && GetDisWithActor(EnemyAIIns) <= 100.f)
	{
		FVector DisNormal = GetActorLocation() - EnemyAIIns->GetActorLocation();
		if (DisNormal.Normalize())
			SetActorLocation(EnemyAIIns->GetActorLocation() + DisNormal * 100.f);
	}
}
...
{
	ItemNum.Add("FirePaper", 2);
	ItemNum.Add("LightPaper", 2);
}
